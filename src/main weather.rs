...
async fn fetch_calendar(config: &Config) -> Result<Vec<CalendarEvent>> {
    ...
    if let Some(token) = &config.google_oauth_token {
...
fn render_display(
    ...
    // ── Calendar panel (right column) ────────────────────────────────────────
    let cal_x = weather_col_w as i32 + 20;
    let cal_col_w = (WIDTH - weather_col_w - 20) as u32;

    draw_text_mut(&mut img, BLACK, cal_x, 64, PxScale::from(20.0), font_bold, "UPCOMING EVENTS");
    draw_line_segment_mut(
        &mut img,
        (cal_x as f32, 90.0),
        ((cal_x + cal_col_w as i32) as f32, 90.0),
        LIGHT_GRAY,
    );
    ...
    if events.is_empty() {
        draw_text_mut(&mut img, DARK_GRAY, cal_x, y, PxScale::from(20.0), font_regular, "No upcoming events");
    }

    let mut last_date_drawn: Option<chrono::NaiveDate> = None;

    for event in events.iter().take(12) {
        if y > HEIGHT as i32 - 30 {
            break;
        }
        ...
        // Date separator
        if last_date_drawn != Some(event_date) {
            last_date_drawn = Some(event_date);

            if y > 96 {
                y += 6;
                draw_line_segment_mut(
                    &mut img,
                    (cal_x as f32, y as f32),
                    ((cal_x + cal_col_w as i32) as f32, y as f32),
                    LIGHT_GRAY,
                );
                y += 6;
            }

            let date_label = if event_date == today {
                "Today".to_string()
            } else if event_date == today + chrono::Duration::days(1) {
                "Tomorrow".to_string()
            } else {
                event.start.format("%a, %b %-d").to_string()
            };

            // Small filled pill for date
            draw_filled_rect_mut(&mut img, Rect::at(cal_x, y).of_size(cal_col_w - 20, 22), Luma([230u8]));
            draw_text_mut(&mut img, BLACK, cal_x + 6, y + 3, PxScale::from(16.0), font_bold, &date_label);
            y += 26;
        }
        ...
        let time_w = 60i32;
        draw_text_mut(&mut img, DARK_GRAY, cal_x, y, PxScale::from(18.0), font_regular, &time_str);

        // Title (clip to available width)
        let title_x = cal_x + time_w;
        let max_chars = ((cal_col_w as i32 - time_w - 20) / 10).max(1) as usize;
        let title = if event.summary.chars().count() > max_chars {
            format!("{}…", &event.summary.chars().take(max_chars - 1).collect::<String>())
        } else {
            event.summary.clone()
        };
        draw_text_mut(&mut img, BLACK, title_x, y, PxScale::from(18.0), font_regular, &title);
...
async fn refresh_data(state: &AppState) -> Result<()> {
    let events = fetch_calendar(&state.config).await.unwrap_or_else(|e| {
